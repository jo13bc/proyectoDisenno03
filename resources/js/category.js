...
function createMenuItem(text, id) {
  return `<li class="card-text">
    <a href="./category.html?id=${id}">${text}</a>
  </li>`;
}
...
function loadMenu(){
  $("#menu").html(
    categories.map(s => createMenuItem(s.name, s.id)).join("")
  );
}
...
function loadCategory() {
  ...
  let titles = document.getElementsByClassName("categoryName");
...
function loadProducts() {
  ...
  container.innerHTML = "";
  let cards = products
    .filter(p => p.categories.find(id => id === category.id))
